...
ThisReport_data$Key <- as.character(ThisReport_data$Key)

# Gender_Extra_2 <- data.frame()
...
#   Gender_Extra_2 <- bind_rows(Gender_Extra_2, missInd)
# }
# save(Gender_Extra_2,file="data/Gender_Extra_2.rda")

load(paste0("data/Gender_Extra_2.rda"))
Gender_Extra_2$Period <- as.character(Gender_Extra_2$Period)
Gender_Extra_2$Observation <- as.character(Gender_Extra_2$Observation)
# Append to master data file
ThisReport_data <- bind_rows(ThisReport_data, Gender_Extra_2)
...
ThisReport_data$Observation <- as.numeric(ThisReport_data$Observation)
ThisReport_data$Period <- as.numeric(ThisReport_data$Period)
